...
class Camera:
    ...
    def get_uv_for_world_points(self, world_points):
        ...
        # FOV Culling: Only allow theta < 105 degrees (Back culling)
        valid_mask = (theta >= 0) & (theta < np.deg2rad(105.0))
        
        return np.stack([u_norm, v_norm], axis=1), valid_mask

# ==========================================
# PART 2: Shader (增加旋转参数)
# ==========================================
...
void main()
{
    ...
    // 强制单相机调试 (不考虑权重，直接显示采样结果，方便看有没有采样错误)
    if (activeCam == 1) {
        if (Weights.x > 0.001) FragColor = colFV; else FragColor = vec4(0.2, 0, 0, 1); return;
    }
    if (activeCam == 2) {
        if (Weights.y > 0.001) FragColor = colRV; else FragColor = vec4(0, 0.2, 0, 1); return;
    }
    if (activeCam == 3) {
        if (Weights.z > 0.001) FragColor = colMVL; else FragColor = vec4(0, 0, 0.2, 1); return;
    }
    if (activeCam == 4) {
        if (Weights.w > 0.001) FragColor = colMVR; else FragColor = vec4(0.2, 0.2, 0, 1); return;
    }
    ...
    if (wTotal < 0.001) {
        ...
        FragColor = vec4(vec3(0.1) + grid*0.1, 1.0);
        return;
    }
...
def create_view_matrix(cam_pos, rot_x, rot_y):
    ...
    t_inv[3, 0] = -cam_pos[0]; t_inv[3, 1] = -cam_pos[1]; t_inv[3, 2] = -cam_pos[2]
    
    ry_inv = np.eye(4, dtype=np.float32)
    ang_y = np.radians(-rot_y); c, s = np.cos(ang_y), np.sin(ang_y)
    ry_inv[0,0]=c; ry_inv[0,2]=-s; ry_inv[2,0]=s; ry_inv[2,2]=c
    
    rx_inv = np.eye(4, dtype=np.float32)
    ang_x = np.radians(-rot_x); c, s = np.cos(ang_x), np.sin(ang_x)
    rx_inv[1,1]=c; rx_inv[1,2]=s; rx_inv[2,1]=-s; rx_inv[2,2]=c
    
    return rx_inv @ ry_inv @ t_inv

# ==========================================
# PART 4: 动态 LUT 生成 (支持在线校准)
# ==========================================

def create_bowl_data_dynamic(cameras, world_range=20.0, rot_offset=0, flip_x=False):
    """
    rot_offset: 旋转角度 (0, 90, 180, 270)
    flip_x: 是否翻转 X 轴
    """
    print(f"[RE-GEN] Generating Mesh with Rot={rot_offset}, FlipX={flip_x}")
    rings, sectors = 80, 128
    flat_percent = 0.40
    ...
    for i in range(rings + 1):
        ...
        for j in range(sectors):
            ...
            x_raw = r * np.cos(angle)
            y_raw = r * np.sin(angle)
            
            # [V37] 动态应用坐标变换
            if flip_x: x_raw = -x_raw
    ...
    for k in range(4):
        u = uv_maps[k][:, 0]
        v = uv_maps[k][:, 1]
        is_in = (u > 0) & (u < 1) & (v > 0) & (v < 1)
        valid = is_in & valid_masks[k]
        dist = np.minimum(np.minimum(u, 1-u), np.minimum(v, 1-v))
        w = np.maximum(0, dist); w = np.power(w, 0.5)
    ...
    for i in range(rings):
        for j in range(sectors):
            curr = i * sectors + j; next_r = (i+1) * sectors + j
            curr_n = i * sectors + (j+1)%sectors; next_rn = (i+1) * sectors + (j+1)%sectors
            indices.extend([curr, next_r, curr_n, curr_n, next_r, next_rn])

    vertex_data = np.hstack([gl_pos, uv_maps[0], uv_maps[1], uv_maps[2], uv_maps[3], weights]).flatten().astype(np.float32)
...
cam_dist = 30.0; cam_pitch = 60.0; cam_yaw = 0.0
last_x, last_y = 0, 0; is_dragging = False
active_cam = 0
global_rot = 0
global_flip = False
need_regen = False
...
def main():
    global active_cam, global_rot, global_flip, need_regen
    
    if not glfw.init(): return
    glfw.window_hint(glfw.CONTEXT_VERSION_MAJOR, 3)
    glfw.window_hint(glfw.CONTEXT_VERSION_MINOR, 3)
    glfw.window_hint(glfw.OPENGL_PROFILE, glfw.OPENGL_CORE_PROFILE)
    window = glfw.create_window(1024, 768, "V37: Press R to Rotate, F to Flip, 1-4 for Cams", None, None)
    ...
    for i, (cf, imf) in enumerate(zip(cam_files, img_files)):
        ...
        cameras.append(Camera(cf, f"Cam{i}", i))
        img = cv2.flip(cv2.cvtColor(cv2.imread(imf), cv2.COLOR_BGR2RGB), 0)
        tid = glGenTextures(1)
        glActiveTexture(GL_TEXTURE0 + i); glBindTexture(GL_TEXTURE_2D, tid)
    ...
    # Initial Gen
    v_data, i_data = create_bowl_data_dynamic(cameras, world_range=20.0, rot_offset=global_rot, flip_x=global_flip)
    vao = glGenVertexArrays(1); glBindVertexArray(vao)
    vbo = glGenBuffers(1); glBindBuffer(GL_ARRAY_BUFFER, vbo); glBufferData(GL_ARRAY_BUFFER, v_data.nbytes, v_data, GL_STATIC_DRAW)
    ebo = glGenBuffers(1); glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo); glBufferData(GL_ELEMENT_ARRAY_BUFFER, i_data.nbytes, i_data, GL_STATIC_DRAW)
    ...
    def key_cb(w, key, scancode, action, mods):
        ...
        if action == glfw.PRESS:
            ...
            if key == glfw.KEY_R: 
                global_rot = (global_rot + 90) % 360
                need_regen = True
            if key == glfw.KEY_F: 
                global_flip = not global_flip
                need_regen = True
            print(f"[STATUS] Cam:{active_cam}, Rot:{global_rot}, Flip:{global_flip}")

    def mouse_cb(w, b, a, m):
        global is_dragging, last_x, last_y
        if b == glfw.MOUSE_BUTTON_LEFT:
            if a == glfw.PRESS: is_dragging = True; last_x, last_y = glfw.get_cursor_pos(w)
            elif a == glfw.RELEASE: is_dragging = False
    
    def cursor_cb(w, x, y):
        global cam_yaw, cam_pitch, last_x, last_y
        if is_dragging:
            cam_yaw += (x - last_x) * 0.5; cam_pitch += (y - last_y) * 0.5
            cam_pitch = max(10, min(89, cam_pitch)); last_x, last_y = x, y
            
    def scroll_cb(w, x, y): global cam_dist; cam_dist -= y * 2.0; cam_dist = max(2.0, min(100.0, cam_dist))

    glfw.set_key_callback(window, key_cb); glfw.set_mouse_button_callback(window, mouse_cb)
    glfw.set_cursor_pos_callback(window, cursor_cb); glfw.set_scroll_callback(window, scroll_cb)

    model_loc = glGetUniformLocation(shader, "model"); view_loc = glGetUniformLocation(shader, "view")
    proj_loc = glGetUniformLocation(shader, "projection"); active_cam_loc = glGetUniformLocation(shader, "activeCam")

    print("=======================================")
    print(" [1-4] Select Camera")
    print(" [R] Rotate World 90 deg")
    print(" [F] Flip X Axis")
    print("=======================================")
    ...
    while not glfw.window_should_close(window):
        ...
        glViewport(0, 0, w, h); glClearColor(0.1, 0.1, 0.1, 1.0); glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
        
        rad_p = np.radians(cam_pitch); rad_y = np.radians(cam_yaw)
        eye = np.array([cam_dist*np.cos(rad_p)*np.sin(rad_y), cam_dist*np.sin(rad_p), cam_dist*np.cos(rad_p)*np.cos(rad_y)], dtype=np.float32)
        
        aspect = w/h if h>0 else 1.0
        proj = create_perspective_matrix(60, aspect, 0.1, 200.0)
        view = create_view_matrix(eye, cam_pitch, cam_yaw)
        
        # LookAt logic inside main loop for robustness
        center = np.zeros(3); up = np.array([0,1,0])
        z = eye - center; z /= np.linalg.norm(z)
        x = np.cross(up, z); x /= np.linalg.norm(x)
        y = np.cross(z, x)
        view = np.eye(4, dtype=np.float32)
        view[0,:3]=x; view[1,:3]=y; view[2,:3]=z
        view[0,3]=-np.dot(x,eye); view[1,3]=-np.dot(y,eye); view[2,3]=-np.dot(z,eye)

        glUniformMatrix4fv(proj_loc, 1, GL_TRUE, proj); glUniformMatrix4fv(view_loc, 1, GL_TRUE, view)
        glUniformMatrix4fv(model_loc, 1, GL_TRUE, np.eye(4, dtype=np.float32)); glUniform1i(active_cam_loc, active_cam)

        glBindVertexArray(vao); glDrawElements(GL_TRIANGLES, len(i_data), GL_UNSIGNED_INT, None)
