...
- **算法原型与验证 (Phase A1)**
  ...
  - [x] **真·零拷贝架构 (True Zero-copy)**：实现了全链路 GPU 内存物理地址共享，端到端延迟降低至 ~15ms。
  - [x] 离线仿真测试 (单目模拟四目分屏验证)
...
- [x] **真·零拷贝 (Zero-copy)**：在实车模式下彻底打通 NVMM -> CUDA -> NVENC 的硬件链路，消除 CPU 映射开销。难度: ⭐⭐⭐⭐
- [ ] **动态外参 (Dynamic Extrinsic - 当前重点)**：集成在线标定算法，实现行驶过程中的外参自动校正与 LUT 实时更新。难度: ⭐⭐⭐⭐⭐
